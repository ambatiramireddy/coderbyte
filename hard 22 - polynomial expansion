function PolynomialExpansion(str) { // works for expanding more than 2 polynomials too
  
  var variable = str.match(/[a-zA-Z]/gi)[0];
  
  function clean(str, arr) { // converts str = "ax^b + cx + d" into [[a, c, d], [b, 1 0]] and store into arr, where x is variable
    ...
    if(str.length === match[0].length) // if this is the last term, then done
      return;
  ...
  function expand(arr) {
    ...
    if(arr.length === 1) // if arr only has 1 polynomial, then done
      return arr[0];
